---
name: "source-command-refactor-claude-md"
description: "Refactor AGENTS.md by extracting sections to dedicated docs with path-scoped rules"
---

# source-command-refactor-claude-md

Use this skill when the user asks to run the migrated source command `refactor-claude-md`.

## Command Template

You are helping refactor a AGENTS.md file to reduce its size while preserving guidance through dedicated documentation files and path-scoped rules.

## Process

1. **Read and analyze** the current AGENTS.md file in its entirety
...
3. **For each candidate section**, recommend:
   ...
   - **Replacement text**: Brief reference to keep in AGENTS.md
...
5. **Create the files** in this order:
   ...
   - Create `.Codex/rules/[name].md` with appropriate globs and `@imports`
   - Update AGENTS.md to replace extracted content with brief reference
...
- **Preserve critical info** in AGENTS.md (database rules, setup, architecture overview)
...
Ask the user which AGENTS.md file to refactor (default to `./AGENTS.md` if not specified).
