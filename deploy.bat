...
call npm run agent-work-os:android:debug
if %errorlevel% neq 0 (
    echo [警告] Android APK 构建失败（可能缺少 JDK/Android SDK，跳过）
    echo        如需构建 Android，请安装 JDK 17+ 和 Android SDK
    set "APK_SKIPPED=1"
) else (
    set "APK_SKIPPED=0"
    echo [OK] Android APK 构建完成
)
...
if "%APK_SKIPPED%"=="0" (
    echo   Android:
    echo     APK 路径: packages\android\app\build\outputs\apk\debug\app-debug.apk
    echo     安装到手机: npm run agent-work-os:android:install
    echo.
)
