...
| `cross_model_review_launched` | `ActionPromptsDropdown.tsx` | User launches the built-in cross-model review action (new session on a different provider reviews the current session's changes) | `writerProvider` (provider of the session under review)<br/>`reviewerModel` (model picked for the reviewer session, null when falling back to writer's provider default) | (pending release) |  |
