...
克隆仓库后，双击根目录的 **`deploy.bat`** 即可完成全部步骤：

```
deploy.bat    ← 双击运行
```

脚本会自动：安装依赖 → 构建桌面应用 → 在桌面创建快捷方式。完成后双击桌面 Nimbalyst 图标即可启动。

> 前提：已安装 [Node.js 22+](https://nodejs.org/) 和 [Git](https://git-scm.com/)
...
```
deploy.bat        # 一键部署脚本（Windows，双击运行）
crystal-run.ps1   # 开发模式启动器（Windows PowerShell）
crystal-run.sh    # 开发模式启动器（macOS/Linux）
packages/
